...
@pytest.mark.parametrize(
    'plain_format_y_invert', [False, True])
def test_y_invert_mocked(mocker, mock_render,
                         quiet, cls, plain_format_y_invert,
                         filename='plain_format_y_invert.gv', format='svg'):
    dot = cls(*[''] if cls.__name__ == 'Source' else [],
              filename=filename, format=format)

    mock_save = mocker.patch.object(dot, 'save', autospec=True)

    assert dot.render(plain_format_y_invert=plain_format_y_invert,
...
@pytest.mark.exe
@pytest.mark.parametrize(
    'plain_text_format, expected_start',
    [('svg', '<?xml version='),
     # https://gitlab.com/graphviz/graphviz/-/blob/main/CHANGELOG.md#1001--2024-02-11
     pytest.param('svg_inline', '<!-- Generated by graphviz version ',
                  marks=pytest.mark.xfail('graphviz.version() < (10, 0, 1)',
                                          reason='needs Graphviz 10 or later',
                                          raises=graphviz.CalledProcessError))])
def test_pipe(dot, plain_text_format, expected_start):
    result = dot.pipe(format=plain_text_format, encoding='utf-8')
